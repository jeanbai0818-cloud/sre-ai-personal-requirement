...
1. **如果当前版本 < 今天日期**：直接用今天日期作为新版本，例如今天 2026.6.6 → 版本改为 `2026.6.6`
2. **如果当前版本 = 今天日期**（已发过一版）：追加 `-1`，即 `2026.6.6-1`；再发则 `-2`……
3. **如果当前版本 > 今天日期**（版本号超前了）：**不能直接用超前日期**，改为今天日期追加后缀，例如今天 6.6 但版本已到 6.8，则用 `2026.6.6-1`（从 `-1` 起，避免与当日第一版冲突）
