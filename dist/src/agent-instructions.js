...
export function createAgentInstructionsBuilder() {
    return ({ availableTools }) => {
        ...
        return [
            ...
            "- 返回 `ok: false, stage: \"token_missing\"` → 把返回值里 `instructions` 字段的文字**一字不改**念给用户，等用户把 token 发过来，写入 workspace 根目录的 `.teable-token.yaml`，再调一次 bootstrap，成功后继续",
            "- 返回 `ok: false, stage: \"member_not_found\"` → 告诉用户：花名册里找不到你的工号，请联系 SRE 工具链维护者。停止本次登记",
            "",
            "**bootstrap 成功后必须做的记录（每次 bootstrap ok 后都检查）：**",
            "1. 读 USER.md，如果里面还没有工号（work_code）或姓名，把 bootstrap 返回的 member.work_code 和 member.name 补写进去",
            "2. 读 MEMORY.md，找有没有关于 personal-requirement 插件的记录。如果没有，在 MEMORY.md 里追加一段：",
            "   - 插件已配置：personal-requirement",
            "   - Teable token 状态：已配置（token 文件路径：.teable-token.yaml，不记录 token 内容本身）",
            "   - 工号：[member.work_code 的值]，姓名：[member.name 的值]，所属组：[member.group 的值]",
            "   - 当季表：[table.name 的值]",
            "   - 作用：收到需求/事项/问题时，走 bootstrap→preview→confirm→record 四步登记流程",
            "3. 如果 MEMORY.md 里已有这段记录，检查当季表名是否和本次 bootstrap 返回的 table.name 一致，不一致则更新",
            "",
