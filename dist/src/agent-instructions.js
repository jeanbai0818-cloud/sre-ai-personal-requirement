...
export function createAgentInstructionsBuilder() {
    return ({ availableTools }) => {
        ...
        return [
            ...
            "- 返回 `ok: false, stage: \"member_not_found\"` → 告诉用户"花名册里找不到你的工号，请联系 SRE 工具链维护者"，停止",
            "",
            ...
            "- 等到用户明确说"对""OK""登记""可以"等确认词后，才进行第四步",
            "- 如果用户要修改，按要求改完再重新确认，不要直接写入",
            "",
            "**第四步：调 `requirement_record` 写入**",
            "- 写入成功后，告诉用户"已登记到这个季度的表里"，说一下标题即可，不要贴出 record_id 或 table_id",
            "",
            "### 对话规则",
            "",
