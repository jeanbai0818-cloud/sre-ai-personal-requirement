...
**不触发（即使内容是反馈类）：**
- 用户只是在转述或讨论，没有要求登记（如"宋超老师说告警延迟了"——无登记动词）
- 用户问的是排查/分析（如"帮我看看这个问题"——动词是"看"不是"记"）
- 日常寒暄、闲聊
- 用户自己 OKR 里的里程碑任务
- 例行的 cron 任务、日报通知
...
> 接下来需要你提供 Teable 个人访问令牌。注意：token 通过 IM 发送存在留存风险（聊天记录、客户端历史、系统日志均可能保存），这是本插件在 IM-only 环境下的已知限制。我会在收到后立即写入本地配置文件，请在发送后尽快删除那条消息，可降低留存风险。
...
### 第四步：写入 — 调 `requirement_record`（必须传 `dry_run: false`）

用第三步确认后的字段调用，**必须显式传 `dry_run: false`**，否则工具只返回预览不写入。

写入成功（返回 `dry_run: false, ok: true`）后告诉用户：

"已登记：[title 的值]（[item_type 的值]）。"

不要把 record_id、table_id、datasheet_id 等技术字段展示给用户。

如果工具返回 `dry_run: true`，说明忘记传 `dry_run: false`，用完整参数重新调用。
