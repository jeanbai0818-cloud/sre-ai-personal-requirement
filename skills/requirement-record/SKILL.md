...
description: 有人找你提了一件事（需求/问题/协作请求/待跟进事项），帮用户把它结构化登记到 Teable 个人季度表。触发词：某某老师反馈了…、业务方说要…、有人让我跟进…、帮我记一下这件事。
...
**token 处理：** 用户把 token 字符串发给你后，把它写入 workspace 根目录的 `.teable-token.yaml`，格式如下，然后再调一次 `requirement_bootstrap` 确认成功：
...
token 只写入这个文件，不得存入 memory、不得在对话里重复提及、不得写进任何注释或日记。
...
读取 workspace 根目录的 `USER.md`（不存在就创建），检查是否已有工号和姓名。

如果缺失，补写进去：

```
- **工号：** [bootstrap 返回的 member.work_code]
- **姓名：** [bootstrap 返回的 member.name]
- **所属组：** [bootstrap 返回的 member.group]
```
...
读取 `MEMORY.md`（不存在就创建），搜索有没有包含 `personal-requirement` 的段落。
...
**如果已有这段**，只检查并更新"当季表"字段，其他不动。季度切换时表名会变，必须同步。

> **为什么要记录这些：** agent 每个 session 重新启动，不记录就不知道 token 有没有配、工号是什么、当季用哪张表。新 session 靠读 MEMORY.md 恢复状态，避免重复引导用户配置 token。
